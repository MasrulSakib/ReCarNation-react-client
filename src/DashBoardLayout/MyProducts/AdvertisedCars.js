...
const AdvertisedCars = () => {
    ...
    const filteredCars = advertisedCars.filter(car => car.paid !== true);
            ...
            <div className="grid grid-cols-1 gap-10 md:container md:mx-auto mx-4">
                {filteredCars.map(car => (
                    <div key={car._id} className="card lg:card-side bg-error shadow-xl md:mx-4">
                        <figure className='lg:w-1/2 lg:h-full'>
                            <img className='h-full w-full' src={car.picture} alt="Album" />
                        </figure>
                        <div className="card-body lg:w-1/2">
                            <h2 className="card-title text-2xl">{car.company} {car.model}</h2>
                            <div className="divider divider-neutral"></div>
                            <p className='text-left'>
                                <span className='font-bold'>Original Price:</span> ${car.original_price} <br />
                            ...
                            <div className="card-actions justify-end">
                                <Link to={`/categorizedcars/${car.company}`}>
                                    <button className="btn-sm md:btn-md btn-outline btn-ghost rounded-none">See More</button>
                                </Link>
                            </div>
                        </div>
                    </div>
